package com.example.newpost.home_fragment.home_terminal.merchantscallback.adapter;
...
import com.example.newpost.home_fragment.home_terminal.merchantstransfer.adapter.ManyRecyclerViewHodler;
...
public class ChooserRecyclerAdapter extends RecyclerView.Adapter<ChooserRecyclerViewHodler>{
    private List<MermachineBean> list;
    ...
    public ChooserRecyclerAdapter(List<MermachineBean> list, Context context){
    ...
    public ChooserRecyclerViewHodler onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.chooser_list_item,parent,false);
        return new ChooserRecyclerViewHodler(view);
    }

    @Override
    public void onBindViewHolder(ChooserRecyclerViewHodler holder, int position) {
        holder.textView_item_number.setText(list.get(position).getPosId().toString());
